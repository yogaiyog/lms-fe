    ...
    <div>
      <div className="mb-6 flex items-start gap-3">
        <div>
          <p className={`mt-1 text-sm ${theme.textMuted}`}>
            Jadwal minggu ini :
            <span className={`font-bold ${theme.text}`}>{getWeekRange().monday.toLocaleDateString("id-ID", { day: "numeric", month: "short" })} – {getWeekRange().sunday.toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</span>
          </p>
        </div>
      </div>
      ...
      {weekSchedules.length === 0 ? (
        <Card theme={theme} className="p-12 flex flex-col items-center text-center border-dashed">
          ...
          <p className={`text-sm mt-1 max-w-sm ${theme.textMuted}`}>Kamu tidak memiliki jadwal mengajar minggu ini</p>
        </Card>
      ) : (
                ...
                <div className="flex items-start gap-4 flex-wrap">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className={`text-xs font-semibold uppercase tracking-wide ${theme.textMuted}`}>{schedule.className}</p>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {schedule.isDone ? (
                      <button onClick={() => onOpenScheduleDetail(schedule)}
                        className="inline-flex items-center gap-1.5 rounded-full bg-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-500 hover:bg-slate-300 transition-colors cursor-pointer">
                        ...
                        {!isCountdownZero && cd && (
                          <div className="mb-1">
                            <div className="flex items-center gap-1">
                              <div className={`flex flex-col items-center rounded-lg px-1.5 py-0.5 min-w-[28px] ${cd.days === 0 ? "bg-red-100" : "bg-blue-100"}`}>
                                <span className={`text-[10px] font-extrabold ${cd.days === 0 ? "text-red-700" : "text-blue-700"}`}>{cd.days}</span>
                                <span className={`text-[7px] font-semibold ${cd.days === 0 ? "text-red-500" : "text-blue-500"}`}>H</span>
                              </div>
                              <span className="text-[9px] text-slate-400">:</span>
                              <div className={`flex flex-col items-center rounded-lg px-1.5 py-0.5 min-w-[28px] ${cd.days === 0 ? "bg-red-100" : "bg-blue-100"}`}>
                                <span className={`text-[10px] font-extrabold ${cd.days === 0 ? "text-red-700" : "text-blue-700"}`}>{String(cd.hours).padStart(2, "0")}</span>
                                <span className={`text-[7px] font-semibold ${cd.days === 0 ? "text-red-500" : "text-blue-500"}`}>J</span>
                              </div>
                              <span className="text-[9px] text-slate-400">:</span>
                              <div className={`flex flex-col items-center rounded-lg px-1.5 py-0.5 min-w-[28px] ${cd.days === 0 ? "bg-red-100" : "bg-blue-100"}`}>
                                <span className={`text-[10px] font-extrabold ${cd.days === 0 ? "text-red-700" : "text-blue-700"}`}>{String(cd.minutes).padStart(2, "0")}</span>
                                <span className={`text-[7px] font-semibold ${cd.days === 0 ? "text-red-500" : "text-blue-500"}`}>M</span>
                              </div>
                              <span className="text-[9px] text-slate-400">:</span>
                              <div className={`flex flex-col items-center rounded-lg px-1.5 py-0.5 min-w-[28px] ${cd.days === 0 ? "bg-red-100" : "bg-blue-100"}`}>
                                <span className={`text-[10px] font-extrabold ${cd.days === 0 ? "text-red-700" : "text-blue-700"}`}>{String(cd.seconds).padStart(2, "0")}</span>
                                <span className={`text-[7px] font-semibold ${cd.days === 0 ? "text-red-500" : "text-blue-500"}`}>D</span>
                              </div>
                            </div>
                          </div>
                        )}
                    ...
                    {!schedule.isDone && (
                      <button
                        onClick={() => onEditSchedule(schedule, schedule.classId)}
                        className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                        </svg>
                      </button>
                    )}
                    {schedule.meetLink && !schedule.isDone && (
